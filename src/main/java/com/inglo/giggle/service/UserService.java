...
public class UserService {
    ...
    public void updatePassport(@UserId Long userId, MultipartFile file) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new CommonException(ErrorCode.NOT_FOUND_USER));
        UserFile userFile = userFileRepository.findById(userId)
                .orElseThrow(() -> new CommonException(ErrorCode.NOT_FOUND_USER_FILE));
        String pastPassportFileUrl = null;
        String currentPassportFileUrl = null;
        boolean isPassportFileExist = userFile.getPassportFileUrl() != null;
        try{
            if (isPassportFileExist) {
                pastPassportFileUrl = userFile.getPassportFileUrl();
            }
            currentPassportFileUrl = imageUtil.uploadPassportImageFile(file, userId);
            userFile.uploadPassportFile(currentPassportFileUrl);
        } catch (Exception e) {
        ...
        try {
            ...
            if (passportNumber == null || name == null || sex == null || dob == null || nationality == null || passportIssueDate == null || passportExpiryDate == null) {
                ...
                user.registerPassport(passportNumber, name.toString(), sex, dob, nationality, passportIssueDate, passportExpiryDate);
                if(isPassportFileExist) {
    ...
    public void updateRegistration(@UserId Long userId, MultipartFile file) {
        ...
        boolean isRegistrationExist = userFile.getRegistrationFileUrl() != null;
        try {
            if (isRegistrationExist) {
                pastRegistrationFileUrl = userFile.getRegistrationFileUrl();
            }
            currentRegistrationFileUrl = imageUtil.uploadRegistrationImageFile(file, userId);
            userFile.uploadRegistrationFile(currentRegistrationFileUrl);
        } catch (Exception e) {
        ...
        try {
            ...
            if (registrationNumber == null || statusOfResidence == null || registrationIssueDate == null) {
                ...
                user.registerRegistration(registrationNumber, statusOfResidence, registrationIssueDate);
                if(isRegistrationExist) {
    ...
    public void updateSocialIntegrationProgram(Long userId, MultipartFile file) {
        ...
        try {
            ...
            if (socialIntegrationProgramScore == null) {
                ...
                user.registerSocialIntegrationProgram(socialIntegrationProgramScore);
            }
        } catch (Exception e) {
            throw new CommonException(ErrorCode.INTERNAL_SERVER_ERROR);
        }
    ...
    public void updateSejongInstitute(Long userId, MultipartFile file) {
        ...
        try {
            ...
            if (sejongInstituteScore == null) {
                ...
                user.registerSejongInstitute(sejongInstituteScore);
            }
        } catch (Exception e) {
            throw new CommonException(ErrorCode.INTERNAL_SERVER_ERROR);
        }
